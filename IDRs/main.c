...
// IDR(s) Solver
int solve_idrs(int n, complex *x, complex *b, op_t Aop, int s, int niter, double tol, int verb)
{
  int i, j, k, iter;
  complex omega, alpha;
  double res_norm;
...
/*
Given A, b, right preconditioner M ≈ A, initial guess x₀

1. y = M * x₀
2. Compute r = b - A * x₀

3. Initialize shadow space P[0:s-1] with random vectors
4. G[0:s-1], U[0:s-1] = zero vectors

5. while ||r|| > tol and iter < maxit:
    a. Solve M * t = r                      // t = M^{-1} r
    b. v = A * t
    c. omega = (vᵗ * r) / (vᵗ * v)
    d. y = y + omega * t
    e. x = M^{-1} * y                        // optional if you need x directly
    f. r = r - omega * v

    g. Orthogonalize r against P using G
    ...
    i. Rotate G[1:s-1] ← G[0:s-2]
       Rotate U[1:s-1] ← U[0:s-2]

    j. iter += 1

6. Return x = M^{-1} y
*/
int solve_idrs_rightpreco(int n, complex *x, complex *b, op_t Aop, op_t Minv, int s, int niter, double tol, int verb)
{
  int i, j, k, iter;
  complex omega, alpha;
  double res_norm;
...
void Id_apply(int n, complex *x, complex *y)
{
...
int main() {
  ...
  solve_idrs_rightpreco(n, x, b, matvec, Id_apply, s, niter, tol, verb);
